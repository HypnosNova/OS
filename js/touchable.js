...
(function($) {
	...
	function Touchable(elem, conf) {
		...
		var log = function(a) {
			if (self.logging && (typeof console !== 'undefined')) {
		...
		this.currentDelta = {
			x: 0,
			y: 0
		}; //measured from previous move event
		...
		addEventListener.call(elem, 'touchstart', function() {
			self.$elem.trigger('touchstart')
		}, true);
		addEventListener.call(elem, 'touchend', function() {
			self.$elem.trigger('touchend')
		}, false);
		addEventListener.call(elem, 'touchmove', function() {
			self.$elem.trigger('touchmove')
		}, false);
